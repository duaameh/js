...
function doubleNumbers(arr){
  ...
  */
  ...
  function makeStrings(arr){
     result=arr.map(ele=> ele.age >18 ? ` ${ele.name}can go to The Matrix`:`${ele.name}is under age`);
     return result;
  }
  console.log(makeStrings([
        {
  ...
  */
  ...
  function doubleValues(arr){
        let doubledArray=arr.map(ele=> ele*2);
...
doubleValues([1,2,3]) ;
    ...
    */
    ...
    function valTimesIndex(arr){
        let result=arr.map((ele,index)=> ele* index);
        console.log(result)
    }
    valTimesIndex([1,2,3]) 
    /*
   ...
   function extractFullName(arr){
    let result=arr.map(ele=> `${ele.first} ${ele.last}`);
    console.log(result)
   }
   extractFullName([{first: 'Elie', last:"Schoppik"},
      {first: 'Tim', last:"Garcia"}, {first: 'Matt', last:"Lane"}, 
